...
void reset_engine() {
  if (actions_tried[BACKWARD] || actions_tried[FORWARD] || actions_tried[RIGHT] || actions_tried[LEFT]) {
    int distance = read_sonar();

    if (distance == MAX_DISTANCE) {
      stop();
      actions_tried[BACKWARD] = false;
      actions_tried[FORWARD] = false;
      actions_tried[RIGHT] = false;
      actions_tried[LEFT] = false;
    }
  }
}
